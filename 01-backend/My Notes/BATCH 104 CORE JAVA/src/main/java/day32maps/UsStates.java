...
public enum UsStates {
    ...
    public static String getStateNameFromAbbreviation(String abbreviation) {
        ...
        return Arrays.
                stream(values()).//Enum'i akışa alip sıraladık(muhakkak values() ile almalıyız çünkü enum.
                        filter(UsStates -> UsStates.getAbbreviation().equals(abbreviation)).//enum abbrv ile girdiğimi karşılaştırdı
                        findFirst().//ayni olanın başını aldı
                        get().//optional yerine bunu kullandık
                        getStateName();//State name i aldık
    ...
    public static String getStateNameFromCapital(String capital) {
        ...
        return Arrays.
                stream(values()).
                filter(t -> t.getCapital().equals(capital)).
                findFirst().//optional data type return eder o sebeple onu tercih etmediğimiz için get() dedik.
                        get().
                getStateName();

    }

    //Example 1: Başkenti verilen state'in kısaltmasini ekrana yazdiran methodu olusturunuz
